...
{
    int ret = driver_register(&fpga_rtintr_driver);
    if (ret < 0) { return ret; }
    ...
    struct resource* res = request_mem_region(RTINTR_BASE, RTINTR_SIZE, "fpga_rtintr");
    if (res == NULL) {
        ret = -EBUSY;
        goto fail_rqmem;
    }
